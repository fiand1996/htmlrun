...
var resolveConfig = _interopDefault(require('@emmetio/config'));
...
var extract = _interopDefault(require('@emmetio/extract-abbreviation'));
var expandAbbreviation = require('@emmetio/expand-abbreviation');
var cssSnippetsResolver = require('@emmetio/css-snippets-resolver');
var parseHTML = _interopDefault(require('@emmetio/html-matcher'));
...
function narrowToNonSpace(editor, from, to) {
	...
	stream.eatWhile(streamReaderUtils.isSpace);
	from = stream.pos;
	...
	if (to) {
		...
		while (!stream.sof() && streamReaderUtils.isSpace(stream.peek())) {
			stream.backUp();
		}
...
class Abbreviation {
	...
	constructor(abbreviation, range, config) {
		...
		this.range = range;
		this.config = config;
		this.ast = expandAbbreviation.parse(abbreviation, config);
		this.snippet = expandAbbreviation.expand(this.ast, config);
 ...
 */
function expandAbbreviation$1(editor, contextAware) {
...
function getSnippetCompletions(editor, pos, config) {
	...
	if (!(syntax in cache)) {
		const registry = expandAbbreviation.createSnippetsRegistry(type, syntax, config.snippets);
...
function getStylesheetSnippets(registry) {
	return cssSnippetsResolver.convertToCSSSnippets(registry).map(snippet => {
...
function getMarkupSnippets(registry, config) {
	return registry.all({ type: 'string' }).map(snippet => ({
		key: snippet.key,
		value: snippet.value,
		preview: removeFields(expandAbbreviation.expand(snippet.value, config)),
		snippet: snippet.key
	}));
}
...
//# sourceMappingURL=emmet-codemirror-plugin.cjs.js.map
