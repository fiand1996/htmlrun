...
class Encryption {
	...
	public function __construct(array $params = array())
	{
		...
		if ( ! isset($this->_key) && self::strlen($key = SALT) > 0)
		{
			$this->_key = $key;
		}

		print_r('info', 'Encryption Class Initialized');
	}
	...
	public function initialize(array $params)
	{
		if ( ! empty($params['driver']))
		{
			if (isset($this->_drivers[$params['driver']]))
			{
				...
				{
					print_r('error', "Encryption: Driver '".$params['driver']."' is not available.");
				}
			}
			else
			{
				print_r('error', "Encryption: Unknown driver '".$params['driver']."' cannot be configured.");
			}
		}
		...
		if (empty($this->_driver))
		{
			...
			print_r('debug', "Encryption: Auto-configured driver '".$this->_driver."'.");
		}
	...
	protected function _mcrypt_initialize($params)
	{
		if ( ! empty($params['cipher']))
		{
			...
			{
				print_r('error', 'Encryption: MCrypt cipher '.strtoupper($params['cipher']).' is not available.');
			}
			else
			{
		...
		if ( ! empty($params['mode']))
		{
			...
			{
				print_r('error', 'Encryption: MCrypt mode '.strtoupper($params['mode']).' is not available.');
			}
			else
			{
		...
		if (isset($this->_cipher, $this->_mode))
		{
			...
			{
				print_r('info', 'Encryption: MCrypt cipher '.strtoupper($this->_cipher).' initialized in '.strtoupper($this->_mode).' mode.');
			}
			else
			{
				print_r('error', 'Encryption: Unable to initialize MCrypt with cipher '.strtoupper($this->_cipher).' in '.strtoupper($this->_mode).' mode.');
			}
		}
	}
	...
	protected function _openssl_initialize($params)
	{
		...
		if ( ! empty($params['mode']))
		{
			...
			{
				print_r('error', 'Encryption: OpenSSL mode '.strtoupper($params['mode']).' is not available.');
			}
			else
			{
		...
		if (isset($this->_cipher, $this->_mode))
		{
			...
			{
				$this->_handle = NULL;
				print_r('error', 'Encryption: Unable to initialize OpenSSL with method '.strtoupper($handle).'.');
			}
			else
			{
				$this->_handle = $handle;
				print_r('info', 'Encryption: OpenSSL initialized with method '.strtoupper($handle).'.');
			}
		}
	}
