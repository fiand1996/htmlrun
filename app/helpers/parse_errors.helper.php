...
function php_check_syntax( $php, $isFile=false )
{
    ...
    if( $isFile OR ( $tf = tempnam( NULL, 'parse-' ) AND file_put_contents( $tf, $php ) !== FALSE ) AND $tf = $php )
    {
        ...
        ob_start();
		global $CONF;
        system( PHP_PATH . ' -c "'.APPPATH.'/inc/php.ini" -l < "'.$php.'"', $ret );
        $output = ob_get_clean();
...
function php_check_runtime( $php )
{
	...
	# Prevent output
	ob_start();
	exec( PHP_PATH . ' -c "'.APPPATH.'/inc/php.ini" < "'.$php.'"', $ret);
	$output = ob_get_clean() ;
	...
	if( $ret !== 0 )
	{
		...
		if( (bool)preg_match('/Warning:\s*(.*?)has been disabled for security reasons in Standard input code on line\s+(\d+)/', implode(",",$ret), $match ) )
		{
			return array(
				'line'    =>    (int)$match[2],
				'method'    =>    $match[1]
